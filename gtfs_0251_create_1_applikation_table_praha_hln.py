...
Creates table: xtra_departures_praha_hln (Part 1)

Usage:
    python gtfs_0251_create_1_applikation_table_praha_nd.py databasename

Tested with feed "Pražská integrovaná doprava (PID)"

Version: 2021-06-29 ... Version 001   
...
sql = "DROP TABLE IF EXISTS xtra_departures_praha_hln"
mycursor.execute(sql)

# Create new "extra" table: All departures from Praha h.n.
sql = f'''
    CREATE TABLE xtra_departures_praha_hln AS (
...
start_time = time()
sql = f"""
    UPDATE xtra_departures_praha_hln
