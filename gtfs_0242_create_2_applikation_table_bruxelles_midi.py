'''
Create table for application "all departures from Bruxelles-Midi".
Creates table: xtra_departures_bruxelles_midi (Part 2)

Usage:
    gtfs_0242_create_2_applikation_table_bruxelles_midi.py databasename

Tested with feed "The NMBS/SNCB Belgian railway company"

Version: 2021-06-30 ... Version 002   
...
sql = "DROP TABLE IF EXISTS xtra_departures_days_bruxelles_midi"
mycursor.execute(sql)

# Create new "extra" table: All departures from Bruxelles-Midi today.
sql = f"""
    CREATE TABLE xtra_departures_days_bruxelles_midi AS (
        SELECT DATE('{today_date}') as day, x.*
        FROM xtra_departures_bruxelles_midi x
...
# Append new "extra" table: All departures from Bruxelles-Midi tomorrow.
start_time = time()
sql = f"""
    INSERT INTO xtra_departures_days_bruxelles_midi
