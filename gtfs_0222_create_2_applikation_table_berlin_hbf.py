'''
Create table for application "all departures from Berlin Hbf"
Creates table: xtra_departures_days_berlin_hbf

Usage:
    gtfs_0222_create_2_applikation_table_berlin_hbf.py databasename

Tested with feed "Schienenfernverkehr" from gtfs.de.

Version: 2021-06-29 ... Version 005   
'''
...
sql = "DROP TABLE IF EXISTS xtra_departures_days_berlin_hbf"
mycursor.execute(sql)

# Create new "extra" table: All departures from Berlin Hbf today.
sql = f"""
    CREATE TABLE xtra_departures_days_berlin_hbf AS (
        SELECT DATE('{today_date}') as day, x.*
        FROM xtra_departures_berlin_hbf x
...
# Append new "extra" table: All departures from Berlin Hbf tomorrow.
start_time = time()
sql = f"""
    INSERT INTO xtra_departures_days_berlin_hbf
